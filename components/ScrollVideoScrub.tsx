...
export default function ScrollVideoScrub() {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      setLoadProgress(Math.round((priorityIndices.size / frameCount) * 100));

      // Load the entire sequence for this device tier before revealing anything -- the whole
      // point is that scrolling should never be able to outrun what's loaded, so "ready" has
      // to mean "everything is ready," not "enough to start." Expands outward from the
      // starting frame purely so the progress bar reflects locally-relevant frames first;
      // the reveal itself still waits for the full set.
      ...
      const workers = new Array(BACKGROUND_LOAD_CONCURRENCY).fill(0).map(async () => {
        while (cursor < loadOrder.length && !cancelled) {
          ...
          const alreadyLoaded = !!imagesRef.current[idx];
          await loadOne(idx);
          if (!alreadyLoaded && !cancelled) {
            loadedCount++;
            setLoadProgress(Math.min(100, Math.round((loadedCount / frameCount) * 100)));
          }
        }
      });
    ...
    // Safety timeout fallback so a genuinely broken/very slow connection can't hang the
    // loader forever -- generous because we're now waiting for the whole sequence, not
    // just a small priority window.
        ...
        {!isInitialized && (
          ...
          >
            ...
            >
              ...
              <span className="text-3xl sm:text-4xl md:text-5xl font-serif tracking-[0.1em] text-[#F6F3EB] font-light leading-none mb-8">
                {t("hero.brand")}
              </span>

              {/* Load progress: the hero only reveals once the entire frame sequence for this
                  device is ready, so scrolling can never outrun what's loaded. A silent
                  multi-second wait reads as broken -- this makes the wait itself the experience. */}
              <div className="w-[160px] sm:w-[200px] h-[1.5px] bg-white/10 rounded-full overflow-hidden">
                <motion.div
              ...
              <span className="text-[10px] font-mono tracking-[0.2em] text-[#F6F3EB]/40 mt-3">
                {loadProgress}%
              </span>
            </motion.div>
          </motion.div>
